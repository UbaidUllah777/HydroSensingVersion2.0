...
for (const singleOwlFocusArea of focusAreas) {
  if (ElowlstageFocusAreas) {
    const areaItem = `
        ...
        <a onclick="loadFocusARea('${singleOwlFocusArea.service_id}')" href="focus-area-detail.html" class="custom-link-hover-effects text-decoration-none" data-cursor-effect-hover="plus">
        <div class="card box-shadow-4">
            <div class="card-img-top position-relative overlay overlay-show">
                <div class="position-absolute bottom-0 left-0 w-100 py-3 px-4 z-index-3">
                    <h4 class="font-weight-semibold text-color-light text-6 mb-1">${singleOwlFocusArea.service_heading}</h4>
  
                </div>
                <img src="img/foucusAreas/services-1.jpg" class="img-fluid" alt="${singleOwlFocusArea.service_heading}" />
            </div>
